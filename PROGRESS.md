...
## ✅ 已完成：Q2 代理模型（高斯过程回归）
- GPR（RBF-ARD 核）+ 闭式 LOOCV：R LOOCV R²=1.0000 / P=0.9996 / U=0.9989
- 8 项物理一致性检查全部通过（复现 Q1 影响规律）
- 详见 `docs/Q2_代理模型总结.md`；预测函数供 Q3 调用

## ✅ 已完成：Q3 多目标优化（Pareto + 熵权-TOPSIS）
- 网格穷举+非支配排序得精确前沿 268 点；NSGA-II 验证一致（距离 0.0006）
- 熵权(前沿) w_R=0.155/w_P=0.366/w_U=0.479；三种决策方法一致：r=4.5、n=4、w≈0.215
- 综合最优方案：(w*,r*,n*)=(0.215,4.500,4)，R=0.7414, P=0.0842, U=0.7722（与样本#64几乎重合）
- 详见 `docs/Q3_多目标优化总结.md`

## ✅ 已完成：Q4 权重敏感性与鲁棒设计
- 权重单纯形网格扫描 1326 点；最高频设计 (0.220,4.5,4) 占 31.2%
- 方法2 minimax regret（合理区间 w_i>=0.15）→ (0.225,4.5,4)；方法4 膝点 → (0.225,4.5,4)，两法一致
- 推荐鲁棒方案 (0.225,4.5,4)：R=0.7412/P=0.0853/U=0.7719，4场景损失仅 0.0007~0.0147
- 详见 `docs/Q4_权重敏感性与鲁棒设计.md`

## ✅ 已完成：Q5 不确定性分析（加工误差 + 工况波动）
- 分层模型（w/r/n→GP；ṁ/T_in→机理缩放；q 无影响）；LSA 弹性 + Sobol'(S1/ST)
- Sobol：R 由流量主导(0.96)、P 由流量+排数(0.50/0.39)、U 由排数+歧管比(0.75/0.20)
- MC(N=10000) 鲁棒方案：R/P/U CV=3.4%/8.1%/0.5%，失效概率(P>1.10标称)=0.6%/8.1%/0%
- 建议：w 容差可放宽，n/r 需控制；优先稳定流量；详见 `docs/Q5_不确定性分析.md`

## 🎉 五问全部完成（详见 docs/ 与 outputs/）
